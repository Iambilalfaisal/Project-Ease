    ...
    return (
        <>
            <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.25rem" }}>
                <button className={styles.backBtn} onClick={onBack}>← Back to Matters</button>
                {!editDetail && (
                    <div style={{ display: "flex", gap: "0.5rem", marginLeft: "auto" }}>
                        <Button variant="ghost" size="sm" onClick={openEdit}>Edit</Button>
                        <button className={styles.actionBtnDanger} style={{ fontSize: "0.8rem" }} onClick={removeMatter}>Delete</button>
                        {matter.case_number && (
            ...
            {lhcStatus.data && (
                <div style={{ margin: "0.5rem 0", padding: "0.75rem 1rem", borderRadius: "var(--radius)", border: "1px solid var(--border)", background: lhcStatus.data.status === "ok" ? "var(--bg-1)" : "rgba(220,38,38,0.06)", fontSize: "0.82rem" }}>
                    ...
                    {lhcStatus.data.message && <div className={styles.muted} style={{ marginTop: "0.25rem" }}>{lhcStatus.data.message}</div>}
            ...
            {editDetail ? (
                ...
                <div className={styles.matterDetailHeader}>
                    <div>
                        <h2 className={styles.detailTitle}>{matter.title}</h2>
                        <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem", flexWrap: "wrap" }}>
                            ...
                            <span className={styles.badgeGray}>{matter.matter_type}</span>
                            {matter.team_name && <span className={styles.badgeGold}>👥 {matter.team_name}</span>}
                        </div>
                    </div>
                    <div className={styles.detailInfoGrid} style={{ marginTop: "1rem" }}>
                        <div className={styles.detailInfoItem}><span className={styles.detailInfoLabel}>Client</span><span>{matter.client_name}</span></div>
                        {matter.court_name    && <div className={styles.detailInfoItem}><span className={styles.detailInfoLabel}>Court</span><span>{matter.court_name}</span></div>}
                        {matter.case_number   && <div className={styles.detailInfoItem}><span className={styles.detailInfoLabel}>Case #</span><span>{matter.case_number}</span></div>}
                        {matter.filing_date   && <div className={styles.detailInfoItem}><span className={styles.detailInfoLabel}>Filed</span><span>{matter.filing_date}</span></div>}
                        {matter.opposing_party && <div className={styles.detailInfoItem}><span className={styles.detailInfoLabel}>Opposing Party</span><span>{matter.opposing_party}</span></div>}
                        {matter.notes         && <div className={styles.detailInfoItem} style={{ gridColumn: "1/-1" }}><span className={styles.detailInfoLabel}>Notes</span><span>{matter.notes}</span></div>}
                        ...
                        {matter.matter_stage && (
                            <div className={styles.detailInfoItem}>
                                ...
                                <span className={styles.badgeAmber} style={{ fontSize: "0.75rem" }}>{matter.matter_stage}</span>
                            </div>
                        )}
                        ...
                        <div className={styles.detailInfoItem}>
                            ...
                            <span style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                <span className={
                                    matter.vakalatnama_status === "Filed"        ? styles.badgeGreen :
                                    matter.vakalatnama_status === "Not Required" ? styles.badgeGray  : styles.badgeAmber
                        ...
                        <div className={styles.detailInfoItem}>
                            ...
                            <span>
                                <span className={
                                    (matter.adjournment_count ?? 0) >= 10 ? styles.limBadgeCritical :
                                    (matter.adjournment_count ?? 0) >= 5  ? styles.badgeAmber : styles.badgeGray
                        ...
                        <div className={styles.detailInfoItem}>
                            ...
                            <span style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                <span className={styles.priorityBadge} data-priority={matter.priority ?? "Normal"}>
                        ...
                        {matter.limitation_date && (() => {
                            ...
                            return (
                                <div className={styles.detailInfoItem} style={{ gridColumn: "1/-1" }}>
                                    ...
                                    <span>
                                        {matter.limitation_date}
                                        {matter.limitation_type && <span className={styles.muted}> ({matter.limitation_type})</span>}
    ...
    return (
        <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "0.75rem 0" }}>
                <span className={styles.muted} style={{ fontSize: "0.82rem" }}>{(matter.documents ?? []).length} document{(matter.documents ?? []).length !== 1 ? "s" : ""} linked</span>
                        ...
                        <div key={catName} className={styles.docHierarchyGroup}>
                            ...
                            {docs.map(doc => (
                                <div key={doc.doc_id} className={styles.docHierarchyRow}>
                                    ...
                                    <button className={styles.queueRemove} title="Unlink from matter" onClick={() => unlinkDoc.mutate(doc.doc_id)}>✕</button>
                                </div>
                            ))}
                        </div>
                    ))}
